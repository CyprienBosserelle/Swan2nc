...
{               
   ...
   // 	static size_t pcount[] = {1, npart};
	...
	//status = nc_def_var (ncid, "yyp", NC_FLOAT,2,pdim, &yyp_id);
   	...
   	status = nc_def_var (ncid, varname, NC_SHORT, 3, var_dimids, &Nin_id);

	status = nc_put_att_float (ncid, Nin_id, "scale_factor", NC_FLOAT,1, &hund);
	...
	status = nc_put_vara_short(ncid, Nin_id, start, count, Hs);
...
{               
   ...
   // 	static size_t pcount[] = {1, npart};
	...
	//status = nc_def_var (ncid, "yyp", NC_FLOAT,2,pdim, &yyp_id);
   	...
   	status = nc_def_var (ncid, varnameA, NC_SHORT, 3, var_dimids, &Nin_id);
	status = nc_def_var (ncid, varnameB, NC_SHORT, 3, var_dimids, &cNin_id);
	...
	status = nc_put_vara_short(ncid, Nin_id, start, count, uu);
	status = nc_put_vara_short(ncid, cNin_id, start, count, vv);
...
void writestep2nc(char outfile[],char varname[], int nx,int ny,float totaltime,short *Hs)
{
	...
	status = nc_inq_varid(ncid, varname, &Nincel_id);
	...
	status = nc_put_vara_short(ncid, Nincel_id, start, count, Hs);
...
void writestep2nc2(char outfile[],char varnameA[], char varnameB[],int nx,int ny,float totaltime,short *uu,short *vv)
{
	...
	status = nc_inq_varid(ncid, varnameA, &Nincel_id);
	status = nc_inq_varid(ncid, varnameB, &cNincel_id);
	...
	status = nc_put_vara_short(ncid, Nincel_id, start, count, uu);
	status = nc_put_vara_short(ncid, cNincel_id, start, count, vv);
